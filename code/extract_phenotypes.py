...
Aim is to create a space in PHENO_PATH dir for each species
...
def get_AMR_df()->pd.DataFrame:
    ...
    if not os.path.exists(PHENO_PATH):
        os.makedirs(PHENO_PATH)

    
    file_path=DATA_PATH+'SIR_readings.xlsx'
...
def create_species_drugs_files(df:pd.DataFrame):
    ...
    for s in species:
        ...
        for d in drugs:
            ...
            sp_df.to_csv(f'{PHENO_PATH}{s.replace(" ","_")}_{d}.csv')
    return None
