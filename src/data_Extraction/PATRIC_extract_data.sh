...
species_file="data/${species_name}_genomes.csv"

echo $header > $species_file
awk -F "," '{print}' $file | grep $species_name >> $species_file

# csv_file='data/genome_summary.csv'
# species_name="Stenotrophomonas maltophiliae"
# df_species=$(awk -F ',' -v species_name="*$species_name*" '{if($2 == species_name) print $0}' $csv_file)
